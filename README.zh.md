...
将 `.agents` 文件夹放入任何工作区。接下来打开它的智能体（Agent）将成为一个接地气、行为可预测的优秀工作伙伴——而不仅仅是一个带有终端访问权限的通用聊天机器人。
...
| `worklog.md` | 之前做了什么——以便下一次会话能从上一次结束的地方精准接续。 |
...
- **11 项精选技能 (Skills)** —— 严格配置的推理循环和格式约束，可有效防止 AI 幻觉，而不是作为一本通用的教科书。
- **技能校准** —— 基于证据的路由策略 (SkillsBench)，可防止堆叠过多技能造成的认知过载。
- **演进治理** —— 智能体可以提出新的技能和词汇建议，但只有人类（用户）才能批准。
- **上下文压缩** —— 自动归档功能，防止记忆文件无限制地膨胀。
...
3. **范围纪律** —— 只触碰任务需要的部分（智能体在维护任务中引入破坏性变更的概率通常会翻三倍）。
4. **先定义成功，再循环执行** —— 在写代码之前，要知道“完成”是什么样子的。

以及一个设计原则：**可预测性高于完美。** 人类不需要一个完美的智能体。他们需要的是一个行为可以被学习、作用范围可以被验证、其失败模式可以随时间被人类补偿的智能体。
