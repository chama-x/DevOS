...
## DevOS vs. 散乱提示词 (.cursorrules, 自定义指令)

用确定的 4 文件上下文引擎，替代散乱的提示词包、单文件 `.cursorrules` 和临时自定义指令。

| 能力 | 原始提示词与技能包 (.cursorrules / CLAUDE.md) | DevOS 引擎 |
|---|---|---|
| **架构** | 单体文本堆叠（浪费上下文窗口） | 4 个模块化核心文件 (~700 tokens) |
| **会话记忆** | 健忘：每次新对话完全重置 | 连续：通过 `worklog.md` 恢复进度 |
| **技能加载** | 一次性加载所有规则（导致幻觉） | 精准校准：按需动态路由最多 2–3 项技能 |
| **范围纪律** | 模糊建议（智能体随机重构代码） | 通过 `GROUNDING.md` 检查进行严格约束 |
| **项目边界** | 隐式或未声明 | 在 `IDENTITY.md` 中明确自主权与红线 |
